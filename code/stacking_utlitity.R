...
m=stan_model("~/Desktop/schools_new/optim.stan")
...
lp_aggregrate=function(lpd_point_test)
{
  test_elpd=matrix(NA, nrow=nrow(lpd_point_test[,,1]), ncol=ncol((lpd_point_test[,,1])))
  S=dim(lpd_point_test)[2]
  for(i in 1:S)
    test_elpd[,i]= apply(lpd_point_test[,i,],  1, log_sum_exp)
  return(test_elpd)
}
